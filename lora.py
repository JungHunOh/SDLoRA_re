...
for eval_dataset in evalsets:
    if model == 'gpt':
        ...
        model_name = 'LLaMA-7B'
    os.system(f'CUDA_VISIBLE_DEVICES={gpu} python {eval_file} --model {model_name} --adapter LoRA --dataset {eval_dataset} --base_model {base_model} --lora_weights ./trained_models/{model}_{dataset}_lora_r{r}_lr{lr}')
